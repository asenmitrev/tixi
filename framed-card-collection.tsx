...
export default function Game() {
  ...
  // Scroll chat to bottom when messages change and show notification if chat is hidden
  useEffect(() => {
    if (chatBottomRef.current) {
      chatBottomRef.current.scrollIntoView({ behavior: "smooth" });
    }
    
    // Show notification when new messages arrive and chat is not visible
    if (messages.length > 0 && !isChatVisible) {
      setHasNewMessages(true);
    }
    
    // Clear notification when chat becomes visible (messages are seen)
    if (isChatVisible) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-4 sm:p-8">
              ...
              <div className="mt-6 p-4 bg-gradient-to-br from-amber-900/40 to-amber-950/40 backdrop-blur-sm rounded-lg border border-amber-700/50">
                <div className="space-y-4">
                  <p className="text-amber-300 font-serif text-sm text-center">
                    Искаш ли да избереш тази карта за историята?
                  </p>
                  <button
                    onClick={() => {
      ...
      <div className="fixed bottom-4 right-4 z-50 md:hidden">
        {!isChatVisible && (
          <button
            ...
            className="relative bg-amber-600 hover:bg-amber-700 text-white p-3 rounded-full shadow-lg"
          >
      ...
      {/* Chat Floating Panel */}
      <div className={`fixed bottom-4 right-4 w-80 max-w-full z-50 transition-all duration-300 ${isChatVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none md:translate-x-0 md:opacity-100 md:pointer-events-auto'}`}>
        <div className="bg-gradient-to-br from-amber-900/80 to-amber-950/90 border border-amber-700/70 rounded-lg shadow-2xl flex flex-col h-96">
          <div className="px-4 py-2 border-b border-amber-700/50 text-amber-200 font-serif text-lg rounded-t-lg bg-amber-900/60 flex justify-between items-center">
            <span>Чат</span>
            <button 
              onClick={() => setIsChatVisible(false)}
              className="text-amber-200 hover:text-white md:hidden"
            >
              <X size={18} />
            </button>
          </div>
